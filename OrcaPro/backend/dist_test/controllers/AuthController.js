...
exports.default = {
    async login(req, res) {
        ...
        try {
            ...
            const senhaValida = await bcryptjs_1.default.compare(senha, user.password);
            if (!senhaValida) {
                res.status(401).json({ error: 'Usuário ou senha inválidos' });
                return;
            }
            ...
            const accessToken = jsonwebtoken_1.default.sign({ id: user.id }, jwtSecret, { expiresIn: '15m' });
            const refreshToken = jsonwebtoken_1.default.sign({ id: user.id, type: 'refresh' }, jwtSecret, { expiresIn: '7d' });
    ...
    async refresh(req, res) {
        ...
        try {
            ...
            const newAccessToken = jsonwebtoken_1.default.sign({ id: decoded.id }, process.env.JWT_SECRET, { expiresIn: '15m' });
    ...
    async register(req, res) {
        try {
            ...
            await prisma.material.createMany({
                data: materiaisPadrao_1.default.map(m => ({ ...m, userId: novoUsuario.id }))
            });
    ...
    async alterarSenha(req, res) {
        try {
            ...
            const senhaValida = await bcryptjs_1.default.compare(senhaAtual, user.password);
            if (!senhaValida) {
    ...
    async forgotPassword(req, res) {
        try {
            ...
            const resetToken = jsonwebtoken_1.default.sign({ userId: user.id }, resetSecret, { expiresIn: '1h' });
            const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:5173';
            const linkReset = `${frontendUrl}/redefinir-senha?token=${resetToken}&id=${user.id}`;
            await (0, emailService_1.enviarEmailResetSenha)(user.email, user.name, linkReset);
            res.json({ message: 'Se este usuário existir e tiver um e-mail cadastrado, você receberá as instruções em breve.' });
        }
        catch (error) {
