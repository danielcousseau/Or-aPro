        ...
        <div className="cliente-card">
            ...
            {materiaisSelecionados.map((mat, index) => (
                <div key={mat.idFalso} style={{ display: 'grid', gridTemplateColumns: '2fr 2fr 1fr 1fr auto', gap: '10px', alignItems: 'end', marginBottom: '12px' }}>
                    ...
                    <section className="form-section">
                        <label>Valor (R$)</label>
                        <input type="number" step="0.01" value={mat.valor} onChange={(e) => onUpdate(mat.idFalso, 'valor', Number(e.target.value))} onFocus={(e) => e.target.select()} required />
                    </section>
                    <section className="form-section">
                        <label>Qtd</label>
                        <input type="number" step="0.01" value={mat.quantidade} onChange={(e) => onUpdate(mat.idFalso, 'quantidade', Number(e.target.value))} onFocus={(e) => e.target.select()} required />
                    </section>
