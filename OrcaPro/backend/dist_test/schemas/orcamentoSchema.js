...
const materialOrcamentoSchema = zod_1.z.object({
    ...
    valor: zod_1.z.coerce.number().min(0, "O valor do material não pode ser negativo"),
    quantidade: zod_1.z.coerce.number().positive("A quantidade deve ser maior que zero")
});
exports.orcamentoSchema = zod_1.z.object({
    titulo: zod_1.z.string().min(3, "O título precisa ter no mínimo 3 caracteres"),
    clienteId: zod_1.z.coerce.number().int().positive("Selecione um cliente válido"),
    tipoMaoDeObra: zod_1.z.string().min(1, "Tipo de mão de obra é obrigatório"),
    maoDeObraValor: zod_1.z.coerce.number().min(0, "Valor da mão de obra inválido"),
