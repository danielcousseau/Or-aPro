...
export default function Dashboard() {
  ...
  useEffect(() => {
    carregarDados();
    api
      .get<Orcamento[]>("/orcamentos/em-producao")
  ...
  const carregarDados = async () => {
    try {
      ...
      const fechados = orcamentosData.filter((orc) =>
        statusFechados.includes(orc.status ?? ""),
      );
      const pendentes = orcamentosData.filter(
        (orc) =>
          !orc.status ||
      ...
      const faturamento = fechados.reduce(
      ...
      const valorNaMesa = pendentes.reduce(
        (acc, orc) => acc + Number(orc.totalFinal),
        0,
      );
      const conversao =
  ...
  const ambientesOrdenados = Object.entries(ambientes).sort(
    (a, b) => b[1] - a[1],
  );
  ...
  const statusCount = orcamentos.reduce<Record<string, number>>((acc, orc) => {
    const status =
      !orc.status || orc.status === "analise" ? "Aguardando" : orc.status;
    ...
    datasets: [
      {
        ...
        backgroundColor: Object.keys(statusCount).map(
          (status) => CORES_STATUS[status] || "#95a5a6",
        ),
        borderWidth: 1,
      },
    ],
      ...
      <section className="dashboard-hero">
        <div>
          ...
          <p className="dashboard-subtitle">
            Acompanhe as métricas e o desempenho geral dos seus projetos.
          </p>
        </div>
      </section>
      ...
      <div className="bento-grid">
        ...
        >
          ...
          <h2 style={{ color: "#f39c12" }}>
            {formatarMoeda(metricas.valorAguardando)}
          </h2>
        </div>
        ...
        <div className="dashboard-card bento-span-3">
          ...
          {orcamentos.length > 0 ? (
            <div
              style={{
                position: "relative",
                height: `${Math.max(200, ambientesOrdenados.length * 48)}px`,
                width: "100%",
              }}
            >
              <Bar
            ...
            <p className="text-center text-soft">
              Cadastre orçamentos para visualizar o gráfico.
            </p>
          )}
        </div>
        <div className="dashboard-card">
            ...
            {emProducao.map((orc) => (
              <div key={orc.id} className="em-producao-card">
                <div className="em-producao-info">
                  <span className="em-producao-cliente">
                    {orc.cliente?.nome}
                  </span>
                  <span className="em-producao-projeto">{orc.titulo}</span>
                  {orc.prazo && (
                    <span className="em-producao-prazo">
                      Prazo: {orc.prazo}
                    </span>
                  )}
                </div>
                <div className="em-producao-direita">
                  <span
                    ...
                    style={{
                      background: CORES_STATUS[orc.status ?? ""] || "#999",
                    }}
                  >
                    {orc.status}
                  </span>
