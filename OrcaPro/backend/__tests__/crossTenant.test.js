...
const { criarUsuarioTeste, limparUsuarioTeste, parseCookies } = require('./helpers');

let userA, userB, cookiesA, cookiesB;

async function login(usuario) {
    const res = await request(app).post('/api/login').send({ usuario, senha: 'senha123' });
...
describe('Isolamento cross-tenant — Materiais', () => {
    ...
    beforeAll(async () => {
        ...
        const res = await request(app).get('/api/materiais').set('Cookie', cookiesA);
