...
const FORM_VAZIO: MaterialFormData = {
  nome: "",
  categoria: "",
  valor: "",
  unidade: "",
...
export default function Materiais() {
  ...
  const [materialEmEdicao, setMaterialEmEdicao] = useState<Material | null>(
    null,
  );
  const [materialParaExcluir, setMaterialParaExcluir] = useState<number | null>(
    null,
  );
  const [materialParaAjustar, setMaterialParaAjustar] =
  ...
  useEffect(() => {
    carregarMateriais();
    api
      .get("/opcoes-customizadas?tipo=material_categoria")
      .then((r) =>
        setCategoriasCustomizadas(r.data.map((o: { nome: string }) => o.nome)),
      )
      .catch(() => {});
    api
      .get("/opcoes-customizadas?tipo=material_unidade")
      .then((r) =>
        setUnidadesCustomizadas(r.data.map((o: { nome: string }) => o.nome)),
      )
      .catch(() => {});
  }, []);
  ...
  const todasCategorias = useMemo(
    () => [
      ...CATEGORIAS_PADRAO,
      ...categoriasCustomizadas.filter((o) => !CATEGORIAS_PADRAO.includes(o)),
    ],
    [categoriasCustomizadas],
  );
  const todasUnidades = useMemo(
    () => [
      ...UNIDADES_PADRAO,
      ...unidadesCustomizadas.filter((o) => !UNIDADES_PADRAO.includes(o)),
    ],
    [unidadesCustomizadas],
  );
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const { name } = e.target;
    const value =
      name === "valor" ? mascaraMoeda(e.target.value) : e.target.value;
  ...
  const handleCategoriaTextoChange = (
  ...
  const handleSalvarCategoriaComoFixo = async (
    e: React.ChangeEvent<HTMLInputElement>,
  ) => {
    ...
    if (checked && formData.categoria?.trim()) {
      try {
        await api.post("/opcoes-customizadas", {
          tipo: "material_categoria",
          nome: formData.categoria.trim(),
        });
        setCategoriasCustomizadas((prev) => [
          ...new Set([...prev, formData.categoria.trim()]),
        ]);
  ...
  const handleSalvarUnidadeComoFixo = async (
    e: React.ChangeEvent<HTMLInputElement>,
  ) => {
    const checked = e.target.checked;
    ...
    if (checked && formData.unidade?.trim()) {
      try {
        await api.post("/opcoes-customizadas", {
          tipo: "material_unidade",
          nome: formData.unidade.trim(),
        });
        setUnidadesCustomizadas((prev) => [
          ...new Set([...prev, formData.unidade.trim()]),
        ]);
  ...
  const handleEditar = (material: Material) => {
    const isCustomCategoria = Boolean(
      material.categoria &&
      !CATEGORIAS_PADRAO.includes(material.categoria) &&
      !categoriasCustomizadas.includes(material.categoria),
    );
    const isCustomUnidade = Boolean(
      material.unidade &&
      !UNIDADES_PADRAO.includes(material.unidade) &&
      !unidadesCustomizadas.includes(material.unidade),
    );
    ...
    setFormData({
      ...
      quantidadeEstoque:
        material.quantidadeEstoque != null
          ? String(material.quantidadeEstoque)
          : "",
      estoqueMinimo:
  ...
  const confirmarAjusteEstoque = async () => {
    ...
    setAjustando(true);
    try {
      const { data: materialAtualizado } = await api.patch(
        `/materiais/${materialParaAjustar.id}/estoque`,
      ...
      toast.success(
        `Estoque de "${materialParaAjustar.nome}" atualizado para ${quantidade}!`,
      );
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const dadosParaEnviar = {
        ...
        quantidadeEstoque:
          formData.quantidadeEstoque !== ""
            ? Number(formData.quantidadeEstoque)
            : null,
        estoqueMinimo:
          formData.estoqueMinimo !== "" ? Number(formData.estoqueMinimo) : null,
      };
      if (materialEmEdicao) {
        const { data: materialAtualizado } = await api.put(
        ...
        const { data: novoMaterial } = await api.post(
          "/materiais",
          dadosParaEnviar,
        );
      ...
      toast.success(
        `Material ${materialEmEdicao ? "atualizado" : "salvo"} com sucesso!`,
      );
      limparFormulario();
  ...
  const estoqueEmAlerta = (m: Material) =>
    m.quantidadeEstoque != null &&
    m.estoqueMinimo != null &&
    m.quantidadeEstoque < m.estoqueMinimo;
              ...
              <div className="form-grid-1-1">
                ...
                <section className="form-section">
                  ...
                  <select
                    name="unidade"
                    value={
                      usandoOutrosUnidade ? "Outros" : formData.unidade || ""
                    }
                    onChange={handleUnidadeChange}
                  >
                    <option value="">Selecione uma unidade...</option>
                    {todasUnidades.map((un) => (
        ...
        <section className="lista-clientes">
          <div className="search-bar">
            <h2>
              ...
              {materiaisFiltrados.length !== 1 ? "materiais" : "material"}
            </h2>
            <input
              type="text"
            ...
            {materiaisFiltrados.length === 0 ? (
              ...
              <div className="grid-cards">
                {materiaisFiltrados.map((material) => (
                  <div
                    key={material.id}
                    className={`cliente-card highlight-primary${estoqueEmAlerta(material) ? " card-alerta-estoque" : ""}`}
                  >
                    <h3 style={{ margin: "0 0 10px 0", fontSize: "1.1rem" }}>
                      {estoqueEmAlerta(material) && (
                        <span
                    ...
                    <p style={{ margin: "4px 0" }}>
                      <strong>Valor:</strong> {formatarMoeda(material.valor)}
                    </p>
                    <p style={{ margin: "4px 0" }}>
                      <strong>Categoria:</strong>{" "}
                      {material.categoria || "Não informada"}
                    </p>
                    <p style={{ margin: "4px 0" }}>
                    ...
                    <p style={{ margin: "4px 0" }}>
                      ...
                      {material.quantidadeEstoque != null ? (
                        <span
                          style={{
                            color: estoqueEmAlerta(material)
                              ? "var(--danger)"
                              : "inherit",
                            fontWeight: estoqueEmAlerta(material) ? 600 : 400,
                          }}
                        >
        ...
        <div className="modal-overlay">
          <div className="modal-content">
            ...
            <p style={{ marginBottom: "4px" }}>
              <strong>{materialParaAjustar.nome}</strong>
            </p>
            <p
              style={{
                color: "var(--text-soft)",
            ...
            >
              Informe a quantidade atual em estoque (sobrescreve o valor
              anterior).
            </p>
            <section className="form-section">
            ...
            <div className="modal-actions" style={{ marginTop: "16px" }}>
              ...
              <button
                type="button"
                onClick={confirmarAjusteEstoque}
                disabled={ajustando}
                style={{ opacity: ajustando ? 0.7 : 1 }}
              >
