...
export default function Historico() {
  ...
  const [orcamentoParaExcluir, setOrcamentoParaExcluir] = useState<
    number | null
  >(null);
  const [termoBusca, setTermoBusca] = useState("");
  const [carregando, setCarregando] = useState(true);
  const [excluindo, setExcluindo] = useState(false);
  const [materiaisAbertos, setMateriaisAbertos] = useState<Set<number>>(
    new Set(),
  );
  ...
  return (
    <div>
      ...
      <div className="search-bar">
        <h2>
          {orcamentosFiltrados.length} orçamento
          {orcamentosFiltrados.length !== 1 ? "s" : ""}
        </h2>
        <input
          type="text"
      ...
      <section className="lista-clientes">
        {carregando ? (
          <p className="text-center text-soft" style={{ padding: "20px" }}>
            Carregando orçamentos...
          </p>
        ) : orcamentosFiltrados.length === 0 ? (
          <p>
            Nenhum orçamento gerado ainda. Crie um novo lá na aba Orçamento!
          </p>
        ) : (
          <div
              ...
              <div key={orc.id} className="cliente-card highlight-primary">
                ...
                <div className="form-grid-1-1">
                  <p>
                    ...
                    {orc.cliente?.nome || "Não informado"}
                  </p>
                  <p>
                    <strong>Telefone:</strong>{" "}
                    {orc.cliente?.telefone || "Não informado"}
                  </p>
                  <p>
                    <strong>Móvel:</strong> {orc.tipoMovel || "-"}
                  </p>
                  <p>
                    <strong>Ambiente:</strong> {orc.ambiente || "-"}
                  </p>
                  <div>
                      ...
                      <>
                        <button
                          ...
                          style={{
                            ...
                            alignItems: "center",
                            gap: "6px",
                            background: materiaisAbertos.has(orc.id)
                              ? "rgba(0, 86, 163, 0.08)"
                              : "var(--panel-soft)",
                            border: "1px solid",
                            borderColor: materiaisAbertos.has(orc.id)
                              ? "var(--primary)"
                              : "var(--border-input)",
                            borderRadius: "20px",
                            padding: "5px 12px",
                        ...
                        >
                          <svg
                            ...
                            style={{
                              transform: materiaisAbertos.has(orc.id)
                                ? "rotate(180deg)"
                                : "rotate(0deg)",
                          ...
                          {materiaisAbertos.has(orc.id) ? "Ocultar" : "Ver"}{" "}
                          {orc.materiais.length}{" "}
                          {orc.materiais.length === 1
                            ? "material"
                            : "materiais"}
                        </button>
                  ...
                  <p>
                    ...
                    <span
                      style={{
                        background:
                          CORES_STATUS[orc.status ?? ""] ||
                          CORES_STATUS["Aguardando"],
                        color: "#fff",
                        padding: "2px 8px",
                        borderRadius: "12px",
                        fontSize: "0.8rem",
        ...
        <div className="modal-overlay">
          <div className="modal-content">
            ...
            <p>
              ...
              ser desfeita e removerá todos os seus dados permanentemente.
            </p>
