...
export default function ResumoValores({
  orcamento,
  totais,
  formasPagamento,
  onChange,
  ...
  useEffect(() => {
    api
      .get("/opcoes-customizadas?tipo=pagamento")
      .then((r) =>
  ...
  const todasOpcoes = [
    ...PAGAMENTOS_PADRAO,
    ...opcoesExtras,
    ...opcoesCustomizadas.filter(
      (o) => !PAGAMENTOS_PADRAO.includes(o) && !opcoesExtras.includes(o),
    ),
  ];

  const isCustom = Boolean(
    orcamento.pagamento && !todasOpcoes.includes(orcamento.pagamento),
  );
    ...
    if (checked && pagamentoAtual.trim()) {
      try {
        await api.post("/opcoes-customizadas", {
          tipo: "pagamento",
          nome: pagamentoAtual.trim(),
        });
        setOpcoesCustomizadas((prev) => [
          ...new Set([...prev, pagamentoAtual.trim()]),
        ]);
          ...
          <div>
            ...
            <div className="input-group-flex">
              ...
              <input
                type="text"
                name="maoDeObraValor"
                value={orcamento.maoDeObraValor}
                onChange={onChange}
                onFocus={(e) => e.target.select()}
                placeholder={
                  PLACEHOLDER_VALOR[orcamento.tipoMaoDeObra] || "R$ 0,00"
                }
              />
            </div>
            {["diaria", "hora", "metro_linear", "metro_quadrado"].includes(
              orcamento.tipoMaoDeObra,
            ) && (
              <div style={{ marginTop: "10px" }}>
                <label
                  style={{ fontSize: "0.9rem", color: "var(--text-soft)" }}
                >
                  Quantidade (Dias / Horas / Metros)
                </label>
                <input
                  type="number"
                  name="maoDeObraQtde"
          ...
          <div>
            ...
            <div className="input-group-flex">
              ...
              <input
                type="text"
                name="lucroValor"
                value={orcamento.lucroValor}
                onChange={onChange}
                onFocus={(e) => e.target.select()}
                placeholder={
                  PLACEHOLDER_VALOR[orcamento.tipoLucro] || "R$ 0,00"
                }
              />
            </div>
      ...
      <div className="cliente-card highlight-success">
        ...
        <div className="form-grid-3">
          <section className="form-section">
            ...
            <select
              name="pagamento"
              value={usandoOutros ? "Outros" : orcamento.pagamento || ""}
        ...
        <section className="form-section" style={{ marginTop: "15px" }}>
          ...
          <textarea
            ...
            onChange={onChange}
            rows={3}
            placeholder="Garantia, condições de instalação, etc..."
          ></textarea>
        </section>
      </div>
    </>
