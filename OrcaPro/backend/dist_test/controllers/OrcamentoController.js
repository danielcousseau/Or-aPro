...
const formatarMoedaBR = (valor) => Number(valor || 0).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
...
exports.default = {
    ...
    async criar(req, res) {
        try {
            const { titulo, clienteId, tipoMaoDeObra, maoDeObraValor, maoDeObraQtde, tipoLucro, lucroValor, lucroQtde, totalFinal, tipoMovel, ambiente, medidas, prazo, pagamento, validade, observacoes, materiais } = req.body;
            ...
            const alertasEstoque = await descontarEstoque(materiais, req.userId).catch(() => []);
            await (0, audit_1.registrar)(req.userId, 'criou', 'Orçamento', novoOrcamento.id, novoOrcamento.titulo);
    ...
    async atualizar(req, res) {
        try {
            ...
            const alertasEstoque = await ajustarDiffEstoque(linhasAtuais, novasMateriais, req.userId).catch(() => []);
            ...
            await (0, audit_1.registrar)(req.userId, 'atualizou', 'Orçamento', orcamentoAtualizado.id, orcamentoAtualizado.titulo);
    ...
    async excluir(req, res) {
        try {
            ...
            await (0, audit_1.registrar)(req.userId, 'excluiu', 'Orçamento', pertence.id, pertence.titulo);
    ...
    async atualizarStatus(req, res) {
        try {
            ...
            await (0, audit_1.registrar)(req.userId, 'atualizou status', 'Orçamento', orcamentoAtualizado.id, `${orcamentoAtualizado.titulo} → ${status}`);
            (0, telegram_1.notificarMudancaStatus)(pertence.cliente, pertence.titulo, status).catch(() => { });
    ...
    async gerarTokenPublico(req, res) {
        try {
            ...
            const token = jsonwebtoken_1.default.sign({ orcamentoId: Number(id) }, jwtSecret, { expiresIn: '7d' });
    ...
    async gerarPDF(req, res) {
        try {
            ...
            const pdfBuffer = await new Promise((resolve, reject) => {
                const doc = new pdfkit_1.default({ size: 'A4', margin: 0, compress: true });
                const chunks = [];
                ...
                const logoPath = path_1.default.join(__dirname, '../../../frontend/public/logo-orcapro.png');
