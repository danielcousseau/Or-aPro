...
export const mascaraMoeda = (valor: string | number | null | undefined): string => {
...
export const desmascararMoeda = (valor: string | number | null | undefined): number => {
    if (!valor) return 0;
    if (typeof valor === 'number') return valor;
    const strValor = String(valor);
    if (!strValor.includes('R$') && !strValor.includes(',')) return Number(strValor) || 0;
